...
// installation tokens — either proxying a workflow dispatch (A2) or returning
// a scoped token (A1, when enabled).
package main
...
func run(logger *slog.Logger, configPath string) error {
	...
	srv := server.New(authn, engine, ghClient, actions.GitHubDispatcher{},
		cfg.Actions, auditLog, logger, cfg.TokenIssuance.Enabled)
